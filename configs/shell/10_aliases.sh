...
if [[ $- == *i* ]]; then
    ...
    fixssh() {
        export SSH_AUTH_SOCK="$(ls -t ~/.ssh/agent/s.*.sshd.* 2>/dev/null | head -n1)"
        ssh-add -l
    }
    alias gdom='git diff origin/main'
    alias grhom='git reset --hard origin/main'
    alias grsom='git reset --soft origin/main'
    alias gsign='git -c gpg.format=ssh -c "user.signingkey=key::$(ssh-add -L | head -n1)" commit -S --amend --no-edit'
    alias gcai="${HOME}/dotfiles/scripts/commit.py --edit "
    alias gcaia="${HOME}/dotfiles/scripts/commit.py --edit --all "
    alias c='code'
    alias cl='claude --dangerously-skip-permissions '
    alias vcl='CLAUDE_CODE_USE_VERTEX=1 CLOUD_ML_REGION=global ANTHROPIC_MODEL="claude-fable-5-1" ANTHROPIC_DEFAULT_HAIKU_MODEL="claude-haiku-4-5" claude --dangerously-skip-permissions '
    ...
    ze() {
        ...
        local name=$(hostname)
        local n=$(pgrep -f "zellij --server .*/${name}\$" | wc -l)
