...
- [ ] Confirm pc's hourly restic backups resume and the pc-restic watchdog check reports OK. Two stacked failures were fixed 2026-07-21: the stale 2026-03-22 repo lock (removed server-side on the NAS), and `truenas.local` no longer resolving since the cutover (pc's repository URL now points at 192.168.1.4). Once a backup succeeds, run `restic-truenas check` on pc to verify repo integrity after the four-month gap.
...
- [x] Close the fleet backup gaps found in the 2026-07-21 audit. Resolved same day: the `mindroom`/`mindroom-mom`/`mindroom-spouse` LXCs all run on pc's Incus with the `dir` storage driver, so pc's (now fixed) hourly restic job captures their full contents under `/var/lib/incus`; `hetzner-saas` gets a nightly k3s state tarball pull (below). Still open, tracked separately: `tank` irreplaceables off-box, `hetzner` Docker named volumes (if any), pi3 state confirmation.
- [x] Back up hetzner-saas (2026-07-21): nightly `nas-backup-hetzner-saas` pulls a tarball of `/var/lib/rancher/k3s/server` (sqlite DB, token, TLS, manifests), `/var/lib/mindroom-saas` (hcloud token), and `/etc/rancher` over SSH into `tank/backups/hetzner-saas` (30-day retention, dump-age watchdog check, key-presence check). No ZFS on that host, so no syncoid. Manual setup done: dataset, key at `/etc/ssh/nas-replication-hetzner-saas-ed25519`, pubkey on root@46.62.174.103 pinned to `from="50.35.43.98"` (home WAN IP; update on change), host key accepted, first 4.8M dump verified.
- [ ] Decide how to cover hetzner-saas customer PVC data: it lives on Hetzner Cloud Volumes via the hcloud CSI, which the k3s state tarball does NOT include. Options: velero, hcloud volume snapshots, or in-cluster backup jobs.
- [ ] After deploying: confirm the `nas-backup-hetzner-saas` timer runs and the watchdog dump-age check reports OK.
