...
The VM must initially run arm64 Ubuntu, allow passwordless sudo for the configured SSH user, expose SSH through IAP, use boot-disk device name `agent-boot`, and attach a blank persistent disk with device name `agent-work`.

Install NixOS.
Pass `--build-on-remote` when the local machine cannot build for the target platform:

```bash
./deploy.py --project PROJECT --zone ZONE --instance INSTANCE --build-on-remote deploy
```
