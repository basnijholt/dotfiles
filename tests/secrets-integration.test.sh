...
install = "- command: '[ -f ./secrets/install ] && (echo \"SECRETS\" && ./secrets/install) || true'"
assert sync in config, "Dotbot does not run sync-secrets.sh as a command block"
assert install in config, "Dotbot does not install cloned secrets on every approved OS"
